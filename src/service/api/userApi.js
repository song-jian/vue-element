...
router.post('/addUser', (req, res) => {
    ...
    conn.query(sql, [params.name, params.account, params.pass, params.checkPass,
                    params.email, params.phone, params.address, params.birth, params.sex], function(err, result) {
        if (err) {
            console.log(err);
        }
        if (result) {
...
router.post('/login', (req, res) => {
    ...
    // console.log(keywords)//传递来的对象
    if (keywords.name) {
        sql_name += " where username ='"+ keywords.name +"'";
        // console.log(sql_name);select * from user where username ='sss'
    }    
